#!/usr/bin/env python3
"""
Vectorized int8 simulation with CORRECT BERT post-LN residual order.
LayerNorm(projection + residual) instead of LayerNorm(projection) + residual.
"""
import numpy as np
import os
...
def softmax_masked(m, mask):
    ...
    return np.floor(e / s * 127.0 + 0.5).clip(-128, 127).astype(np.int8)
...
def layernorm_residual_i32(acc_in, res_in_i8, gamma, beta):
    """BERT post-LN: LayerNorm(acc_in + res_in_i8)"""
    combined = acc_in.astype(np.float64) + res_in_i8.astype(np.float64)
    mean = combined.mean(axis=1, keepdims=True)
    var = ((combined - mean)**2).mean(axis=1, keepdims=True)
    normed = (combined - mean) / np.sqrt(var + 1e-12)
...
for ex in range(N):
    ...
    for L in range(2):
        ...
        proj_acc = i8mm(ctx, qd["Wo_q"], qd["Wo_b"], acc=True)
        
        # BERT post-LN: LayerNorm(Wo_output + residual)
        attn_out = layernorm_residual_i32(proj_acc, cur,
                                          wd["attn_ln_gamma"], wd["attn_ln_beta"])
        
        f1 = i8mm(attn_out, qd["ff1_q"], qd["ff1_b"])
        f1g = gelu_i8(f1)
        f2_acc = i8mm(f1g, qd["ff2_q"], qd["ff2_b"], acc=True)
        
        # BERT post-LN: LayerNorm(ff2_output + residual)
        cur = layernorm_residual_i32(f2_acc, attn_out,
...
print(f"\n=== INT8 FIXED ACCURACY: {100*correct/N:.2f}% ({correct}/{N}) ===", flush=True)
