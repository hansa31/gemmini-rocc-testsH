...
//******************************** CONVERSIONS FOR FP16


#include <time.h>
#include <stdint.h>

// If CLOCK_MONOTONIC isn't defined, use CLOCK_REALTIME instead
#ifndef CLOCK_MONOTONIC
    #define CLOCK_MONOTONIC CLOCK_REALTIME
#endif

#define TOP_K 10        // accuracies
...
static inline uint64_t get_time_ns(void) {
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);        //POSIX system call that gets the current time from a clock source
    return (uint64_t)ts.tv_sec * 1000000000ULL + (uint64_t)ts.tv_nsec;
}

//typedef int8_t elem_t_i;

int8_t images_int8[NUM_IMAGES * IMAGE_SIZE];
elem_t images[NUM_IMAGES * IMAGE_SIZE];

/*
void load_images(const char *filename, elem_t  *buffer, size_t size) {
    FILE *fp = fopen(filename, "rb");
    if (!fp) {
        printf("❌ Failed to open %s\n", filename);
        exit(1);
    }
    fread(buffer, sizeof(elem_t ), size, fp);
    fclose(fp);
}*/

void load_and_convert_images(const char *filename, elem_t *buffer, size_t size) {
    FILE *fp = fopen(filename, "rb");
    if (!fp) {
    ...
    fread(images_int8, sizeof(int8_t), size, fp);
    fclose(fp);

    for (size_t i = 0; i < size; i++) {
        buffer[i] = (float)images_int8[i];
    }
}


void load_and_convert_images_fp16(const char *filename, elem_t *buffer, size_t size) {
    FILE *fp = fopen(filename, "rb");
    if (!fp) {
        printf("Error opening file\n");
        exit(1);
    ...
    for (size_t i = 0; i < size; i++) {
        buffer[i] = float_to_fp16(((float)images_int8[i]));
    }
}

//This file is made by Hansa to remove all the readcycle instructions.
...
int main (int argc, char * argv[]) {
    ...
    //load_images("images.bin", images, NUM_IMAGES * IMAGE_SIZE);
    load_and_convert_images_fp16("images200.bin", images, NUM_IMAGES * IMAGE_SIZE);


    
    // --- 🧪 Print first 5 pixels of first image ---
    // 🧪 Print first 5 pixels
    printf("🔍 First 5 pixels of image 0:\n");
    for (int i = 0; i < 5; i++) {
        int base = i * 3;
        printf("Pixel %d: R=%f, G=%f, B=%f\n", i,
               images[base + 0], images[base + 1], images[base + 2]);
    }

    int num_batches = NUM_IMAGES/4;

    int labels[NUM_IMAGES];      //change the number of files

    // Load labels from file
    FILE *fp2 = fopen("labels200.txt", "r");
    for (int i = 0; i < NUM_IMAGES; i++) {       //change the number of files
    ...
    for (int batch_idx = 0; batch_idx < num_batches; batch_idx++) {
        ...
        // Warm up stdout so the timing isn't affected by lazy initialization
        setvbuf(stdout, NULL, _IONBF, 0);

        uint64_t start = get_time_ns();

        elem_t* current_images = &images[batch_idx * 4 * IMAGE_SIZE];
        ...
        //start = read_cycles();
            ...
            //end = read_cycles();
            //uint64_t end_conv1 = rdtime();
            //im2col_cycles += end_conv1 - start_conv1;
        ...
        for (int batch = 0; batch < fc_53_params.batch_size; batch++) {
            ...
            printf("Prediction: %u (score: %f)\n", max_idx, fp16_to_float(max_prob));
        }

        // This loop is added by Hansa to check the top k Prediction IDs
        for (int batch = 0; batch < fc_53_params.batch_size; batch++) {

            int label = labels[batch + batch_idx*4];

            printf("%d\n",label);

            // Arrays to hold top 10 scores and their indices
            float top_scores[TOP_K] = {0};
            ...
            printf("Top 10 predictions for image %d:\n", batch + 1);
            for (int k = 0; k < TOP_K; k++) {
                printf("  Class ID: %d, Score: %.6f\n", top_indices[k], top_scores[k]);
            }
            printf("\n");
    ...
    */
        ...
        for (int i = 0; i < fc_53_params.batch_size; i++) {
            printf("Predicted class %d: %d, Expected class: %d\n", i + 1, preds[i], correct[i]);
            /*
    ...
    printf("Top-1 Accuracy: %.2f%%\n", (100.0f * top1_correct) / NUM_IMAGES);
    printf("Top-5 Accuracy: %.2f%%\n", (100.0f * top5_correct) / NUM_IMAGES);
    printf("Top-10 Accuracy: %.2f%%\n", (100.0f * top10_correct) / NUM_IMAGES);
