...
int main (int argc, char * argv[]) {
    ...
    for (int batch_idx = 0; batch_idx < num_batches; batch_idx++) {
        ...
        for (int batch = 0; batch < BATCH_SIZE; batch++) {
            ...
            if (top_indices[0] == label) top1_correct++;
            for (int k = 0; k < 5; k++) {
                if (top_indices[k] == label) { top5_correct++; break; }
            }
            for (int k = 0; k < 10; k++) {
                if (top_indices[k] == label) { top10_correct++; break; }
            }
        }
        ...
        if ((batch_idx + 1) % 100 == 0 || batch_idx == 0) {
            ...
            printf("  [%d/%d images] Top-1: %.2f%%, Top-5: %.2f%%, Top-10: %.2f%%\n",
